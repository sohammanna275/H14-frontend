...
const UpdateScores = () => {
  ...
  const match = matches.find(
    (m) => m.matchID === parseInt(selectedMatchId)
  );
  ...
  if (match) {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            scoreA:
                scoreForm.runsA !== ""
                    ? Number(scoreForm.runsA)
                    : match.scoreA,

            wicketsA:
                scoreForm.wicketsA !== ""
                    ? Number(scoreForm.wicketsA)
                    : match.wicketsA,

            oversA:
                scoreForm.oversA !== ""
                    ? Number(scoreForm.oversA)
                    : match.oversA,

            // Team B
            scoreB:
                scoreForm.runsB !== ""
                    ? Number(scoreForm.runsB)
                    : match.scoreB,

            wicketsB:
                scoreForm.wicketsB !== ""
                    ? Number(scoreForm.wicketsB)
                    : match.wicketsB,
            oversB:
                scoreForm.oversB !== ""
                    ? Number(scoreForm.oversB)
                    : match.oversB,
        };
                ...
                {matches.length === 0 ? (
                    ...
                    matches.map((m) => (
                        ...
                        >
                            ...
                            <p>
                                Team A: {m.scoreA !== null ? `${m.scoreA}/${m.wicketsA} in ${m.oversA} overs` : "Yet to bat"}
                            </p>
                            <p>
                                Team B: {m.scoreB !== null ? `${m.scoreB}/${m.wicketsB} in ${m.oversB} overs` : "Yet to bat"}
                            </p>
                        </div>
                    ))
                )}
